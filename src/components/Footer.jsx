import mailImg from "../assets/images/mail-24px.png";
import enterImg from "../assets/images/arrow_forward-24px.png";

export default function Footer() {
  return (
    <>
      <footer className="flex flex-col  w-full">
        {/* 深色區塊 */}
        <div className="flex w-full gap-[33px] lg:flex-row flex-col justify-between items-center lg:px-[122px] p-[30px] bg-primary-600 ">
          <div className="flex gap-5">
            <img
              src="https://raw.githubusercontent.com/hexschool/webLayoutTraining1st/refs/heads/master/student-week1/logo-light.svg"
              alt="logo"
              className="w-10 h-10"
            />
            <h2 className="h2-text font-normal">訂閱你我的甜蜜郵件</h2>
          </div>
          {/* 信箱輸入欄 */}
          <div className="flex bg-primary-100 max-w-[380px] w-full justify-between items-center">
            </div>
  {/* 左邊圖示 */}
  <div className="flex justify-between  h-[64px] lg:w-[380px] w-full items-center   bg-white ">
    <div className="pl-5">
    <img src={mailImg} alt="mail" className="h-6 w-6 " />
    </div>
   <div className="flex h-[64px] w-[56px] bg-secondary-200 hover:bg-secondary-400 items-center justify-center cursor-pointer">
    <img src={enterImg} alt="enter" className="h-6 w-6" />
  </div>
  
  </div>

  

  {/* 右邊按鈕 */}
  
</div>

        {/* 淺色區塊 */}
        <div className="flex flex-col justify-between gap-[30px] lg:px-[122px] px-[30px] py-[39px] bg-primary-100 ">
        <div className="flex w-full lg:justify-between ">
          <div className="flex flex-col lg:justify-between gap-10 ">
            <img
              src="https://raw.githubusercontent.com/hexschool/webLayoutTraining1st/refs/heads/master/student-week1/logotype-sm-dark.svg"
              alt="logo"
              className="w-[177px] h-auto"
            />
            <p className="footer-text flex  leading-6">
              07-1234-5678 <br />
              sweetaste@email.com 800 <br />
              高雄市新興區幸福路 520 號
            </p>
          </div>

          <div className="relative w-[60px] h-[218.69px] headline hidden lg:flex">
            <h3 className="absolute right-0 top-0">今天是個——</h3>
            <h3 className="absolute left-0 bottom-0">吃甜點的好日子。</h3>

          </div>
        </div>

          <div className="flex flex-col gap-[30px]">
            <div className="flex flex-col lg:flex-row lg:justify-between gap-[23px]">
              <div className="flex gap-2">
                <img
                  src="https://raw.githubusercontent.com/hexschool/webLayoutTraining1st/refs/heads/master/student-week1/ic-line%40.svg"
                  alt="line"
                  className="w-8 h-8"
                />
                <img
                  src="https://raw.githubusercontent.com/hexschool/webLayoutTraining1st/refs/heads/master/student-week1/ic-facebook.svg"
                  alt="fb"
                  className="w-8 h-8"
                />
                
              </div>
              <div className="font-times justify-start text-primary-600 flex lg:justify-center items-center gap-0.5">
                <p>©2018</p>
                <img
                  src="https://raw.githubusercontent.com/hexschool/webLayoutTraining1st/refs/heads/master/student-week1/logotype-sm-dark.svg"
                  alt="logo"
                  className="h-[13px] w-auto"
                />
                <p>All rights reserved.</p>
              </div>
            </div>
          </div>

          
        </div>
      </footer>
    </>
  );
}
